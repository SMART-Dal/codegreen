...
const codegreenAnalyzer_1 = require("./codegreenAnalyzer");
const energyHotspotProvider_1 = require("./energyHotspotProvider");
const energyReportProvider_1 = require("./energyReportProvider");
let analyzer;
let hotspotProvider;
let reportProvider;
let energyStatusBarItem;
function activate(context) {
 console.log('CodeGreen Energy Analyzer extension is now active!');
 // Initialize providers
 analyzer = new codegreenAnalyzer_1.CodeGreenAnalyzer(context);
 hotspotProvider = new energyHotspotProvider_1.EnergyHotspotProvider(context);
 reportProvider = new energyReportProvider_1.EnergyReportProvider(context);
 ...
 title: `Optimize ${(hotspot === null || hotspot === void 0 ? void 0 : hotspot.function) || 'Function'}`,
 placeHolder: 'Select an optimization strategy'
 });
 if (selected) {
 vscode.window.showInformationMessage(`Applying ${selected.label} to ${(hotspot === null || hotspot === void 0 ? void 0 : hotspot.function) || 'the function'}...`);
 }
 });
 ...
 context.subscriptions.push(analyzeCommand, clearCommand, showReportCommand, toggleAutoCommand, optimizeCommand, predictCommand, hotspotTreeProvider, reportTreeProvider, saveListener, energyStatusBarItem);
