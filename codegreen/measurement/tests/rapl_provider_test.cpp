...
private:
 ...
 if (!provider) {
 std::cout << " Intel RAPL not available on this system\n";
 ...
 auto provider = create_intel_rapl_provider();
 if (!provider) {
 std::cout << " RAPL provider not available, skipping initialization test\n";
 return true;
 }
 ...
 std::cout << " RAPL provider not available, skipping reading test\n";
 return true;
 }
 ...
 std::cout << " Energy readings are not monotonic (may indicate counter wraparound)\n";
 }
 ...
 auto provider = create_intel_rapl_provider();
 if (!provider || !provider->initialize()) {
 std::cout << " RAPL provider not available, skipping accumulation test\n";
 return true;
 }
 ...
 std::cout << " Measuring energy accumulation over 2 seconds...\n";
 ...
 std::cout << " Very low energy delta - system may be idle\n";
 } else {
 std::cout << " Reasonable energy accumulation detected\n";
 }
 
 if (average_power > 500.0) { // Sanity check
 std::cout << " Very high power consumption - check readings\n";
 }
 ...
 auto provider = create_intel_rapl_provider();
 if (!provider || !provider->initialize()) {
 std::cout << " RAPL provider not available, skipping load test\n";
 return true;
 }
 ...
 std::cout << " Low power increase - may indicate measurement issues\n";
 } else {
 ...
 auto provider = create_intel_rapl_provider();
 if (!provider || !provider->initialize()) {
 std::cout << " RAPL provider not available, skipping wraparound test\n";
 return true;
 }
 ...
 std::cout << " High non-monotonic rate - possible counter issues\n";
 } else {
