...
# Global measurement collector - properly declared
_measurement_collector: Optional[MeasurementCollector] = None
_collector_lock = threading.Lock()

# Global session management - properly declared
...
class MeasurementSession:
    ...
    def add_measurement(self, checkpoint_id: str, checkpoint_type: str, 
        ...
        measurement = CheckpointMeasurement(
            ...
            context=context,
            timestamp=time.perf_counter(),  # Consistent high-precision timing
...
# Removed _ensure_collector_initialized() - now using inline thread-safe initialization
...
def measure_checkpoint(checkpoint_id: str, checkpoint_type: str, 
    ...
    # Thread-safe lazy initialization with proper double-checked locking
    if _measurement_collector is None:
        with _collector_lock:
            if _measurement_collector is None:
                _measurement_collector = MeasurementCollector()
    
    # Now safely use the collector (guaranteed to be initialized)
