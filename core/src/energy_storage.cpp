...
bool SQLiteEnergyStorage::execute_statement(const std::string& sql) {
    ...
    // Check if it's a CREATE TABLE statement (allow for whitespace)
    std::string trimmed_sql = sql;
    trimmed_sql.erase(0, trimmed_sql.find_first_not_of(" \t\n\r"));
    bool is_create_table = (trimmed_sql.substr(0, 12) == "CREATE TABLE");
    
    // Only allow whitelisted statements or CREATE TABLE statements
    if (!is_create_table && safe_statements.find(sql) == safe_statements.end()) {
