...
## Checkpointing and Energy Attribution

NEMB uses a signal-generator model where checkpoints act as lightweight timestamp markers. Complete details on checkpoint implementation, format specification, threading, and energy correlation algorithms are documented in:

**See:** [Checkpointing Architecture](checkpointing-architecture.md)

Key characteristics:
- Sub-microsecond checkpoint overhead (~100-200ns)
- Thread-local invocation tracking for zero contention
- Binary search + linear interpolation for energy attribution
- Support for recursive calls and multi-threading
